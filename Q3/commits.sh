...
########################################
# 3. Search the CSV for a matching row
#    The CSV columns are:
#    1) BUGID
#    2) DESCRIPTION
#    3) BRANCH
...
#    7) GITHUBURL
########################################
# We look for the row whose 3rd column == $BRANCH (case-insensitive).
...
########################################
# We'll parse the row into:
#   BUGID, DESCRIPTION, BRANCH, DEVNAME, PRIORITY, REPO, GITHUBURL
OLDIFS=$IFS
...
# Format: BugID:CurrentDate:Branch:DevName:Priority:Description[:DevComment]
...
if [ $? -ne 0 ]; then
  echo "ERROR: Could not write git log to 
